...
class ReservaSerializer(serializers.ModelSerializer):
    ...
    clase_precio = serializers.DecimalField(
        source='clase.precio',
        max_digits=10,
        decimal_places=2,
        read_only=True
    ...
    def validate(self, data):
        ...
        if clase.cupos_disponibles() <= 0:
            raise serializers.ValidationError(
                "La clase ha alcanzado su límite de capacidad. "
                "Podés anotarte en Lista de Espera."
            )

        # Validación de suspensión
        if paciente.suspendido:
            raise serializers.ValidationError(
                "Tu cuenta se encuentra suspendida para nuevas reservas "
                "por inasistencias previas."
            )
        ...
        if Reserva.objects.filter(
            ...
            raise serializers.ValidationError(
                "Ya tenés una reserva activa para esta clase."
            )
