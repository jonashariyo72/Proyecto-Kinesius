...
export default function Clases({ precioPorDefecto = 15000 }) {
  ...
  const guardar = async e => {
    ...
    try {
      ...
      if (data?.sala?.length) {
        setFormError(data.sala[0])

      } else if (data?.kinesiologo?.length) {
        setFormError(data.kinesiologo[0])

      } else if (data?.__all__?.length) {
        setFormError(data.__all__[0])

      } else if (data?.non_field_errors?.length) {
        setFormError(data.non_field_errors[0])

      } else {
        setFormError('No se pudo guardar la clase.')
      }
    } finally {
      setGuardando(false)
    }
    ...
    <div>
      ...
      <div style={s.grid}>
        ...
        {clases.map(c => (
          ...
          <div
            key={c.id}
            style={{ ...s.card, opacity: c.activa ? 1 : 0.5 }}
          >
      ...
      {modal && (
        <div style={s.overlay}>
          ...
          <div style={s.modal}>
            ...
            {formError && (
              <p style={s.errorTxt}>{formError}</p>
            )}
            ...
            <form onSubmit={guardar} style={s.formGrid}>
              ...
              <div style={s.campo}>
                ...
                <input
                  ...
                  onInvalid={e =>
                    e.target.setCustomValidity(
                      `La capacidad debe ser mayor o igual a ${minCapacidad}`
                    )
                  }
                  required
                />
